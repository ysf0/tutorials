...
ArrayList stores a native array itself. In some programming languages (like "C") the size of native arrays can be managed dynamically. (Additional note: Normally there is not dynamic arrays in C but we can manage dynamic arrays using pointers). In some languages like Java, there is no such a dynamic array. In this tutorial we will assume that the language does not supports dynamic arrays.
